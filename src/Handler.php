...
class Handler implements HasAssocCallbackParams
{
    ...
    public function handle() {
        ...
        if (($message = array_search($this->req->getText(), Handler::BACK_MESSAGES)) !== false){
            $backKey = 'last_' . $this->req->getFrom()['id'];
            TgBot::deleteMsg($this->req);
            $message = setting($backKey, 'start');
            return $this->handleMethod($message);
        }
        if (($message = array_search($this->req->getText(), Handler::COMMANDS)) !== false){
            TgBot::deleteMsg($this->req);
            return $this->handleMethod($message);
        }
        $action = $this->req->getCommandEntity();
        if (is_null($action)) {
            $action = $this->getCachedCommand() ?: Handler::START;
        }
